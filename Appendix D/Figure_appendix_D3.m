...
RESULT_names={'KSHNK_PFAM_results.mat','KSHNK_AFAM_results.mat'};%%EXACT NAMES OF THE RESULTS FILES TO LOAD

lin_vs_nonlin=[0,0,0,0]; % = 0 if corresponding model non-linear, 1 otherwise
Model_names=char('\phi_{\pi}=1.2 \gamma^{\tau}=1','\phi_{\pi}=1.2 \gamma^{\tau}=0.5'); %NAMES OF THE MODEL VARIANTS TO APPEAR IN THE LEGEND OF THE GRAPHS
...
VAR_IRFs_linear = {'Y','C','I','H','HS','HH','CSl','CHl','PIE','R','B','ZH','ZS','BH','BS'};%,'RR','RnRn','PIEPIE','tauC','tauK','tauW','BG','BGC','BGW',}; %% NAMES OF ENDOGENOUS VARIABLES OF INTEREST FOR THE IRFS (LINEAR MODELS) - NEED TO HAVE EXACTLY SAME NAME AS IN THE .MOD FILE
...
for mm=1:Num_models
if jj==6||jj==16||jj==18
subplot(Rows_figure,Column_figure,jj), plot(zeros(IRFPeriods,1),vLinestyle{mm},'Color',vColors{mm},'LineWidth',LinewidthDefault); 
    else
