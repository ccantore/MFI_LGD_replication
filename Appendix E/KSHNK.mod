...
%% Version of the code with capital portability - Appendix E
...
Sconst=CS+IS+ZS/P-(W*HS+lambdass*RK*K(-1)/PIS+Rn(-1)*BS(-1)/(P*PIS)+profits/PIS -tauS-taxS/(P)); %%To double check steady state is correct
...
CN+ZN/P = W*HN+lambdasn*RK*K(-1)/PIS + Rn(-1)*BN(-1)/(P*PIN) - tauN - taxN/(P) ;
...
CH+ZH/P=W*HH+lambdash*RK*K(-1)/PIS+BH(-1)/(P*PIH)*Rn(-1)-taxH/(P)+tauH;
