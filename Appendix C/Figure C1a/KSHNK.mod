...
@#define real_debt = 1
% 0: nominal goverment debt
% 1: real government debt
...
theta_pie  = 0.8; %Taylor rule elasticity of interest rate to inflation
...
@#elseif real_debt == 1
Sconst=CS+IS+ZS-(W*HS+RK*K(-1)/PIS+R(-1)*BS(-1)/PIS+profits/PIS -tauS-taxS); %%To double check steady state is correct
@#endif
...
@#elseif real_debt == 1       
CN+ZN = W*HN + R(-1)*BN(-1)/PIN - tauN - taxN ;
@#endif
...
@#elseif real_debt == 1
CH+ZH=W*HH+BH(-1)/PIH*R(-1)-taxH+tauH; 
@#endif
...
@#elseif real_debt == 1
B_obs=B/STEADY_STATE(Y); 
ZH_obs=ZH/STEADY_STATE(Y); 
BS_obs=BS/STEADY_STATE(Y);
BH_obs=BH/STEADY_STATE(Y); 
BN_obs=BN/STEADY_STATE(Y); 
ZN_obs=ZN/STEADY_STATE(Y); 
ZS_obs=ZS/STEADY_STATE(Y);
G_obs=G/STEADY_STATE(Y);
tax_obs=tax/STEADY_STATE(Y);
@#endif
LI_obs=log(LI);
LS_obs=log(LShare);
KS_obs=log(KShare);
